...
import fs from "fs";
import { PostFileNode, FolderNode } from "@/lib/FileSystem.model";
import { get } from "http";
import { getPostSlug } from "@/lib/extract.file.lib";
...
const root = takeDirectoryReturnList(process.cwd(), "data", -1);
root.then((r) => r.print());

function flattenTree(node: FolderNode | PostFileNode): (FolderNode | PostFileNode)[] {
